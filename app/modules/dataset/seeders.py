...
class DataSetSeeder(BaseSeeder):
    ...
    def run(self):
        ...
        ds_metrics = DSMetrics(number_of_models="5", number_of_csv="50")
        ...
        # Assume there are 12 csv files, create corresponding FMMetaData and csvModel
        fm_meta_data_list = [
            FMMetaData(
        ...
        csv_models = [
            CSVModel(data_set_id=seeded_datasets[i // 3].id, fm_meta_data_id=seeded_fm_meta_data[i].id)
            for i in range(12)
        ]
        seeded_csv_models = self.seed(csv_models)

        # Create files, associate them with csvModels and copy files
        load_dotenv()
        working_dir = os.getenv("WORKING_DIR", "")
        src_folder = os.path.join(working_dir, "app", "modules", "dataset", "csv_examples")
        for i in range(12):
            file_name = f"file{i+1}.csv"
            csv_model = seeded_csv_models[i]
            dataset = next(ds for ds in seeded_datasets if ds.id == csv_model.data_set_id)
            user_id = dataset.user_id

            dest_folder = os.path.join(working_dir, "uploads", f"user_{user_id}", f"dataset_{dataset.id + 100}")
