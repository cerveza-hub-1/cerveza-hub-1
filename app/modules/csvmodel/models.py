...
class CSVModel(db.Model):
    ...
    files = db.relationship("Hubfile", backref="csv_model", lazy=True, cascade="all, delete")
    fm_meta_data = db.relationship("FMMetaData", uselist=False, backref="csv_model", cascade="all, delete")

    def __repr__(self):
        return f"CSVModel<{self.id}>"
